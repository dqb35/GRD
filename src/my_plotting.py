...
'''
plotter的调用：
fig = plotter(experiment, a_score, validationD.reshape(-1, feats), lossFinal:(10000,),
                                              labelsFinal:(10000,), result, recons.reshape(-1, feats),
                                              samples.reshape(-1, feats), None, dim=dim, plot_test=True, epoch=e)
'''
...
def plotter(model, dataset, ground_truth, anomaly_score, labels, results=None, ae_recon=None, diff_sample=None, preds=None, dim=0, plot_test=True, epoch=0, set='test'):
    ...
    print(timestamps) # ground_truth就是validationD.reshape(-1, feats)
    ...
    FP = [1 if preds[i] and not labels[i] else 0 for i in range(0, timestamps)] # 误报
    FN = [1 if not preds[i] and labels[i] else 0 for i in range(0, timestamps)] # 漏报
    ...
    plt.savefig(f'{folder}/dim_{dim}_epoch_{epoch}.pdf') # 为了确保不失真，保存为pdf格式
    plt.close()
    return fig
