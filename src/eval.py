...
def pak_protocol(scores, labels, threshold, max_k=100):
    ...
    area_under_f1 = auc(ks, f1s) # 给定阈值后,不同k值对应不同的f1分数,求不同f1下面积得到f1_k
    max_f1_k = max(f1s) # 在该阈值下,能得到的最大f1分数(一般是k=0的情况)
...
def evaluate(score, label, validation_thresh=None):
    ...
    thresholds = np.arange(0, score.max(), score.max()/50) #0.001 从0到最大异常分数遍历(共有50个值)
    ...
    if validation_thresh:
        ...
        f1 = max(f1s) # 选择不同阈值下最大的f1_K分数来认为是本次模型的的最终f1分数
