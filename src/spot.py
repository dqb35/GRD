...
class SPOT:
    ...
    def __init__(self, q=1e-4):
        ...
        SPOT object
        """
        self.proba = q # 检测水平（风险），由用户选择
        self.extreme_quantile = None
        self.data = None
        self.init_data = None
        self.init_threshold = None # 初始阈值
        self.peaks = None
        self.n = 0
        self.Nt = 0
    ...
    def initialize(self, level=0.98, min_extrema=False, verbose=True):
        ...
        n_init = self.init_data.size # size是一个属性，返回数组的元素总数，SMD:n_init=28479

        S = np.sort(self.init_data)  # we sort X to get the empirical quantile
        self.init_threshold = S[int(level * n_init)]  # 初始阈值

        # initial peaks
        self.peaks = self.init_data[self.init_data > self.init_threshold] - self.init_threshold # 超过初始阈值的峰值（数据点）
        ...
        g, s, l = self._grimshaw() # 使用Grimshaw的技巧计算GPD参数估计
        self.extreme_quantile = self._quantile(g, s) # 计算给定GPD参数的分位数
    ...
    def run(self, with_alarm=True, dynamic=True):
        ...
        with_alarm (默认值为True): 如果设置为False，则SPOT将在假设没有异常值的情况下调整阈值
        dynamic (默认值为True): 如果设置为False，则SPOT不会动态更新阈值，而是使用初始化阶段计算的初始阈值。
        ...
        for i in range(self.data.size):
            ...
            if not dynamic:
                if self.data[i] > self.init_threshold and with_alarm:
                    self.extreme_quantile = self.init_threshold # 将当前阈值设置为初始阈值
                    alarm.append(i) # 将数据点的索引添加到报警列表
            else:
