...
class ConditionalDiffusionTrainingNetwork(nn.Module):
	def __init__(self,nr_feats, window_size, batch_size, noise_steps, denoise_steps, train=True):
		super().__init__()
		self.dim = min(nr_feats, 16) # nr_feats即dims
		self.nr_feats = nr_feats
		self.window_size = window_size
		self.batch_size = batch_size

		self.training = train
		self.timesteps = noise_steps # 加噪
		self.denoise_steps = denoise_steps # 去噪
	...
	def forward(self, x):
		...
		x = x.reshape(-1,  1, self.window_size, self.nr_feats)		# (batch_size,1,100,38)
		if self.training:
			t = torch.randint(0, self.timesteps, (x.shape[0],), device=device).long() # 每个窗口的加噪步骤(从0到noise_steps不等)，t.shape:(batch_size,)
